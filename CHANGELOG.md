...
- Added `Add-AzDataTableLargeEntity`, `Get-AzDataTableLargeEntity` and `Remove-AzDataTableLargeEntity` for working with entities that exceed the Azure Table Storage size limits (64 KiB per string property, 1 MiB per entity). Oversized string properties are split into chunk properties recorded in a `SplitOverProps` JSON manifest, and entities that are still too large are distributed over multiple rows marked with `OriginalEntityId`, `PartIndex` and `PartCount`; reads reassemble the original entity transparently, fetching any rows the caller's filter did not match, and removes deletes all part rows. The existing entity cmdlets are unaffected.

### Fixed

- `Get-AzDataTableLargeEntity` now returns whole entities regardless of which of their rows the filter matched. Reassembly previously merged only the rows the query happened to return, so an entity split over several rows came back in pieces whenever the filter did not sweep up its `{RowKey}-part{n}` rows — including for the most obvious query of all, the entity's own RowKey. Missing rows are now fetched, by a RowKey range scoped to the entity's partition, before reassembly.
- `Get-AzDataTableLargeEntity` no longer silently truncates a split property. When a chunk named in the `SplitOverProps` manifest was absent, the remaining chunks were concatenated and stored as though they were the whole value, producing data that could not be distinguished from the original. A chunk that cannot be found now raises an `IncompleteEntityException` instead, as does a group of part rows that is missing rows outright.
- `Add-AzDataTableLargeEntity` writes the `SplitOverProps` manifest to the entity's first row rather than its last. The manifest was appended to the entity after the chunk properties, so it landed on whichever row the writer filled last; a reader holding only the first row therefore had every chunk and nothing describing them, and skipped rejoining without reporting anything.
- `Add-AzDataTableLargeEntity` records a `PartCount` on every part row. Part indexes are contiguous whether or not the tail of an entity is present, so rows 0-2 of a five-row entity were indistinguishable from a complete three-row one. Rows written before this property existed are still read correctly, by validating the manifest instead.
